...
- (void)didMoveToWindow {
    ...
    UIFont *labelfont = [UIFont fontWithName:@"HelveticaNeue" size:16];
    ...
    for (int component=0; component<self.numberOfComponents; component++) {
        ...
        if (text) {
            ...
            frame.size = [text sizeWithAttributes:@{NSFontAttributeName: labelfont}];
            
            //NSDictionary *sizeAttrs = [[NSDictionary alloc] initWithObjectsAndKeys:NSFontAttributeName, labelfont, nil];
            //frame.size = [longestString sizeWithAttributes:sizeAttrs];
            
            // center it vertically
            frame.origin.y = (ceilf(self.frame.size.height) / 2) - (ceilf(frame.size.height) / 2) + 27;
